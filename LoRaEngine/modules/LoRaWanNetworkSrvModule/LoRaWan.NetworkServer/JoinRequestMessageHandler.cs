        ...
        public void DispatchRequest(LoRaRequest request)
        {
            ...
            async Task ProcessJoinRequestAsync(LoRaRequest request)
            {
                ...
                try
                {
                    ...
                    StaticLogger.Log(devEUI, $"join request received", LogLevel.Information);
                    ...
                    {
                        StaticLogger.Log(loRaDevice.DevEUI, "join refused: missing AppKey for OTAA device", LogLevel.Error);
                    ...
                    {
                        StaticLogger.Log(devEUI, "join refused: AppEUI for OTAA does not match device", LogLevel.Error);
                        request.NotifyFailed(loRaDevice, LoRaDeviceRequestFailedReason.InvalidJoinRequest);
                        return;
                    }
                    ...
                    {
                        StaticLogger.Log(devEUI, "join refused: invalid MIC", LogLevel.Error);
                    ...
                    if (!string.IsNullOrEmpty(loRaDevice.DevNonce) && loRaDevice.DevNonce == devNonce)
                    {
                        ...
                        {
                            StaticLogger.Log(devEUI, "join refused: join already processed by another gateway", LogLevel.Information);
                        }
                        else
                        {
                            StaticLogger.Log(devEUI, "join refused: DevNonce already used by this device", LogLevel.Error);
                        }
                    ...
                    {
                        StaticLogger.Log(devEUI, $"join refused: trying to join not through its linked gateway, ignoring join request", LogLevel.Information);
                        loRaDevice.IsOurDevice = false;
                    ...
                    if (!timeWatcher.InTimeForJoinAccept())
                    {
                        ...
                        StaticLogger.Log(devEUI, $"join refused: processing of the join request took too long, sending no message", LogLevel.Information);
...
#pragma warning restore CS0618 // #655 - This Rxpk based implementation will go away as soon as the complete LNS implementation is done
                        ...
                        {
                            StaticLogger.Log(loRaDevice.DevEUI, $"failed to retrieve the join channel index for device", LogLevel.Error);
                        }
                    }
                    ...
                    {
                        StaticLogger.Log(devEUI, $"join refused: join request could not save twin", LogLevel.Error);
...
#pragma warning restore CS0618 // #655 - This Rxpk based implementation will go away as soon as the complete LNS implementation is done
                        {
                            StaticLogger.Log(loRaDevice.DevEUI, "could not resolve DR and/or frequency for downstream", LogLevel.Error);
                    ...
                    {
                        StaticLogger.Log(devEUI, $"processing of the join request took too long, using second join accept receive window", LogLevel.Debug);
...
#pragma warning restore CS0618 // #655 - This Rxpk based implementation will go away as soon as the complete LNS implementation is done
                    ...
                    if (loRaDevice.DesiredRX2DataRate.HasValue)
                    {
                        ...
                        {
                            StaticLogger.Log(devEUI, $"twin RX2 DR value is not within acceptable values", LogLevel.Error);
                        }
                    }
                    ...
                    {
                        StaticLogger.Log(devEUI, $"twin RX1 offset DR value is not within acceptable values", LogLevel.Error);
                    }
                    ...
                    {
                        StaticLogger.Log(devEUI, $"twin RX delay value is not within acceptable values", LogLevel.Error);
                    }
                    ...
                    if (joinAccept != null)
                    {
                        ...
                        if (StaticLogger.LoggerLevel <= LogLevel.Debug)
                        {
                            ...
                            StaticLogger.Log(devEUI, $"{LoRaMessageType.JoinAccept} {jsonMsg}", LogLevel.Debug);
                        }
                        else if (StaticLogger.LoggerLevel == LogLevel.Information)
                        {
                            StaticLogger.Log(devEUI, "join accepted", LogLevel.Information);
                        }
                    }
                }
                catch (Exception ex) when
                    (ExceptionFilterUtility.True(() => StaticLogger.Log(devEUI ?? ConversionHelper.ByteArrayToString(request.Payload.DevAddr),
