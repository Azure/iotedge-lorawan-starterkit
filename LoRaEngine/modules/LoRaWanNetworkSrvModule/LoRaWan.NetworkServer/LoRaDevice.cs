...
namespace LoRaWan.NetworkServer
{
    ...
    public sealed class LoRaDevice : IDisposable, ILoRaDeviceRequestQueue
    {
        ...
        /// <summary>
        /// Gets or sets the <see cref="LoRaTools.Regions.LoRaRegion"/> of the device
        /// Relevant only for <see cref="LoRaDeviceClassType.C"/>
        /// </summary>
        public LoRaRegion LoRaRegion
        {
...
                return true;
        ...
        internal async Task<bool> UpdateAfterJoinAsync(string devAddr, string nwkSKey, string appSKey, string appNonce, string devNonce, string netID, LoRaRegion? region, string preferredGatewayID)
        {
            ...
            if (region.HasValue)
            {
                this.region.Set(region.Value);
                if (this.region.IsDirty())
                {
                    reportedProperties[this.region.PropertyName] = region.Value.ToString();
                }
            }
