...
namespace LoRaWan.IntegrationTest
{
    ...
    public class SensorDecodingTest : IntegrationTestBaseCi
    {
        ...
        public async Task SensorDecoder_HttpBased_ValueSensorDecoder_DecodesPayload()
        {
            ...
            await this.RunDecoderTestsAsync(device);
        }

        [RetryFact]
        public async Task SensorDecoder_HttpBased_ValueSensorDecoder_DecodesPayload_With_Universal_Decoder()
        {
            var device = this.TestFixtureCi.Device31_OTAA;
            await this.RunDecoderTestsAsync(device);
        }
        ...
        private async Task RunDecoderTestsAsync(TestDeviceInfo device)
        {
            this.LogTestStart(device);
            await this.ArduinoDevice.setDeviceModeAsync(LoRaArduinoSerial._device_mode_t.LWOTAA);
            await this.ArduinoDevice.setIdAsync(device.DevAddr, device.DeviceID, device.AppEUI);
            ...
            // this join is the first of the standard test series. Therefore we allow a lot of retries.
            var joinSucceeded = await this.ArduinoDevice.setOTAAJoinAsyncWithRetry(LoRaArduinoSerial._otaa_join_cmd_t.JOIN, 20000, 20);
            Assert.True(joinSucceeded, "Join failed");
