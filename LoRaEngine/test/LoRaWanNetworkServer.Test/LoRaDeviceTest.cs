...
namespace LoRaWan.NetworkServer.Test
{
    ...
    public class LoRaDeviceTest
    {
        ...
        public void When_ResetFcnt_In_NonZero_FcntUp_Or_FcntDown_Should_Have_HasFrameCountChanges_True()
        {
            ...
            Assert.Equal(LoRaRegionEnum.NotSet, target.LoRaRegion);
        ...
        public async Task When_Initialized_ABP_Device_Has_Fcnt_Should_Have_Non_Zero_Fcnt_Values()
        {
            ...
            Assert.Equal(LoRaRegionEnum.NotSet, loRaDevice.LoRaRegion);
            Assert.False(loRaDevice.HasFrameCountChanges);
        }
            ...
            [CombinatorialValues("EU868", "3132", "eu868", "US915", "us915", "eu")] string regionValue)
        {
            ...
            if (string.Equals(LoRaRegionEnum.EU868.ToString(), regionValue, StringComparison.InvariantCultureIgnoreCase))
                Assert.Equal(LoRaRegionEnum.EU868, loRaDevice.LoRaRegion);
            else if (string.Equals(LoRaRegionEnum.US915.ToString(), regionValue, StringComparison.InvariantCultureIgnoreCase))
                Assert.Equal(LoRaRegionEnum.US915, loRaDevice.LoRaRegion);
            else
                Assert.Equal(LoRaRegionEnum.NotSet, loRaDevice.LoRaRegion);
        }
    }
